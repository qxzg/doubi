...
#	Version: 1.0.4
#	Author: Toyo
#	Blog: https://doubmirror.cf/shell-jc8/
#=================================================
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/qxzg/doubi/master/gfw_push.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	wget -N --no-check-certificate "https://raw.githubusercontent.com/qxzg/doubi/master/gfw_push.sh" && chmod +x gfw_push.sh
